...
public class AuthorizationHeaderFilter
	...
	public GatewayFilter apply(Config config) {
		...
		return (exchange, chain) -> {
			...
			/* 설명. request header에 담긴 값을 로그로 확인 */
			...
			/* 설명. "Authorization"이라는 키 값으로 request Header에 담긴 것 추출(사용자가 들고 온 JWT 토큰) */
	...
	private boolean isJwtValid(String jwt) {
		...
		/* 설명. 기본적으로 우리 서버에서 맏는 토큰인지와,토큰 안에 sub 라는 등록된 클레임이 있는지 확인 */
		try {
			subject = Jwts.parser()
